...
async function refreshLive(){
  const r = await fetch('api/last.php?device='+encodeURIComponent(DEFAULT_DEVICE)+'&_t='+Date.now());
  ...
  const t_pv   = t.pv_kwh;
  const t_feed = t.feed_in_kwh;
  const t_bin  = t.batt_in_kwh;
  const t_bout = t.batt_out_kwh;
  const t_imp  = t.grid_import_kwh;
  const t_cons = t.consumption_kwh;
...
async function refreshHistory(){
  const r = await fetch('api/daily.php?device='+encodeURIComponent(DEFAULT_DEVICE)+'&days=30&_t='+Date.now());
  const j = await r.json();
