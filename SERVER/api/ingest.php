...
$device = (string) ($payload['device'] ?? 'home');
if (!preg_match('/^[A-Za-z0-9_.:-]{1,64}$/', $device)) {
  http_response_code(400); echo json_encode(['ok'=>false,'error'=>'invalid device']); exit;
}
$authenticatedDevice = (string) ($_SERVER['HTTP_X_PVDASH_DEVICE'] ?? '');
if ($PVDASH_REQUIRE_AUTH && $authenticatedDevice !== $device) {
  http_response_code(401); echo json_encode(['ok'=>false,'error'=>'device mismatch']); exit;
}
...
if ($ROLL_BY_PV && $prevRow) {
  ...
  $upPrev = daily_totals_upsert_statement($pdo, true);
  $nowts = time();
...
$upsertDay = daily_totals_upsert_statement($pdo, true);
