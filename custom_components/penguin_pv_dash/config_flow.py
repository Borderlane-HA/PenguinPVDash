...
class PenguinPVDashConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            # Create entry with data and options separated
            return self.async_create_entry(
                title="PenguinPVDash",
                data={
                    # server_url and api_key are optional now (server script later)
                    CONF_SERVER_URL: user_input.get(CONF_SERVER_URL),
                    CONF_API_KEY: user_input.get(CONF_API_KEY),
                    CONF_DEVICE_ID: user_input.get(CONF_DEVICE_ID) or self.hass.config.location_name or "ha",
                },
                options={
                    ...
                    CONF_BATT_POWER_ENTITY: user_input.get(CONF_BATT_POWER_ENTITY),
        ...
        unit_options = [
            SelectOptionDict(label="kW", value="kW"),
        ...
        schema = vol.Schema({
            ...
            vol.Optional(CONF_BATT_POWER_ENTITY): EntitySelector(EntitySelectorConfig(domain=["sensor"])),
            ...
            vol.Optional(CONF_OUTPUT_UNIT, default=DEFAULT_OUTPUT_UNIT): SelectSelector(SelectSelectorConfig(
                options=unit_options, mode="dropdown"
            )),
...
class PenguinPVDashOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            vol.Optional(CONF_BATT_POWER_ENTITY, default=data.get(CONF_BATT_POWER_ENTITY)): EntitySelector(EntitySelectorConfig(domain=["sensor"])),
            ...
            vol.Optional(CONF_OUTPUT_UNIT, default=data.get(CONF_OUTPUT_UNIT)): SelectSelector(SelectSelectorConfig(
                options=unit_options, mode="dropdown"
            )),
