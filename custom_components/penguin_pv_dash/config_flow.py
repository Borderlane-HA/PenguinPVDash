    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            return self.async_create_entry(
                title="PenguinPVDash",
                data={
                    CONF_SERVER_URL: user_input.get(CONF_SERVER_URL),  # optional
                    CONF_API_KEY: user_input.get(CONF_API_KEY),        # optional
...
class PenguinPVDashOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            vol.Optional(CONF_OUTPUT_UNIT, default=data.get(CONF_OUTPUT_UNIT, DEFAULT_OUTPUT_UNIT)): SelectSelector(SelectSelectorConfig(
                options=unit_options, mode="dropdown"
            )),
